...
esp_matter::cluster_t *create(esp_matter::endpoint_t *endpoint, config_t *config,
                              esp_matter::cluster_flags_t flags) {
  ...
  esp_matter::cluster_t *bridged_device_basic_information_cluster =
      esp_matter::cluster::bridged_device_basic_information::create(
          endpoint, &(default_config), esp_matter::cluster_flags::CLUSTER_FLAG_SERVER);
  ON_NULL_PRINT_RETURN(bridged_device_basic_information_cluster, nullptr,
                       "Failed to create the bridged device basic information cluster");
