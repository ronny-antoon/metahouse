...
namespace mh_matter
{
    uint32_t get_device_type(uint16_t endpoint_id)
    {
        ...
        // If the device is a Matter Aggregator, Bridged Node or Root Node, the device type is the second element in the array
        if (deviceTypeIdArraySize != 1 && (deviceTypeIdArray[0] == ESP_MATTER_AGGREGATOR_DEVICE_TYPE_ID || deviceTypeIdArray[0] == ESP_MATTER_BRIDGED_NODE_DEVICE_TYPE_ID || deviceTypeIdArray[0] == ESP_MATTER_ROOT_NODE_DEVICE_TYPE_ID))
        {
    ...
    esp_err_t attribute_cb_internal(esp_matter::attribute::callback_type type, uint16_t endpoint_id, uint32_t cluster_id,
                                    uint32_t attribute_id, esp_matter_attr_val_t *val, void *priv_data)
    {
        if (type == esp_matter::attribute::callback_type_t::PRE_UPDATE)
        {
            return attribute_cb_external(get_device_type(endpoint_id), endpoint_id, cluster_id, attribute_id, val, priv_data);
        }
    ...
    esp_err_t identify_cb_internal(esp_matter::identification::callback_type type, uint16_t endpoint_id, uint8_t effect_id,
        ...
        case chip::DeviceLayer::DeviceEventType::kFabricRemoved:
        {
            ...
            if (chip::Server::GetInstance().GetFabricTable().FabricCount() == 0)
            {
                ...
                if (!commissionMgr.IsCommissioningWindowOpen())
                {
                    ...
                    {
                        ON_ERR_PRINT(ESP_ERR_NOT_FINISHED, "Failed to open commissioning window, err:%" CHIP_ERROR_FORMAT, err.Format());
                    }
                }
            }
            break;
